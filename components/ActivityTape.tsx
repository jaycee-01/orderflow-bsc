...
export function ActivityTape() {
  return (
    <div className="w-full bg-fog-light/40 border-y border-fog-light/80 overflow-hidden py-3 text-xs font-mono select-none">
      <div className="mx-auto max-w-7xl px-4 flex items-center gap-4">
        
        {/* Ticker Badge */}
        <div className="flex items-center gap-2 shrink-0 bg-signal/10 border border-signal/30 text-signal px-2.5 py-1 rounded text-[11px] font-semibold uppercase tracking-wider font-mono">
          ...
          <div className="flex whitespace-nowrap animate-ticker gap-8 hover:[animation-play-state:paused] font-mono">
            {[...INITIAL_ACTIVITY_TAPE, ...INITIAL_ACTIVITY_TAPE, ...INITIAL_ACTIVITY_TAPE].map((act, i) => (
              <div key={`${act.id}-${i}`} className="inline-flex items-center gap-3 text-bone-muted text-[11px]">
                ...
                <span className="text-signal font-mono font-semibold">{act.amount}</span>
                <span className="text-[10px] bg-delta-green/20 text-delta-green px-1.5 py-0.5 rounded font-mono">
                  {act.status}
                </span>
                ...
                <span className="text-fog-light/80">|</span>
              </div>
            ))}
          </div>
        </div>
