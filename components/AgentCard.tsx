...
export function AgentCard({ agent }: { agent: AgentData }) {
  ...
  return (
    <Link
      ...
      className="group relative flex flex-col justify-between rounded-lg border border-fog-light bg-fog p-5 transition-all duration-200 hover:-translate-y-1 hover:border-signal/70 hover:shadow-md block"
    >
      {/* Header Eyebrow */}
      <div>
        ...
        <div className="grid grid-cols-3 gap-2 py-2 px-3 bg-fog-light/40 rounded border border-fog-light font-mono text-xs mb-4">
          <div>
            ...
            <span className="text-signal-text font-bold flex items-center gap-1">
              <ShieldCheck className="h-3 w-3 inline text-signal-text" /> {agent.summaryValue}%
            </span>
          </div>
          <div>
